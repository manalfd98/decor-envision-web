...
export const Specificproductpage = () => {
    ...
    const addtocart = async (item) => {
        if (loggeduser) {
            ...
            const checkIfProductExists = cartProducts.some((item) => item.producttitle === product.producttitle);
    
            if (checkIfProductExists) {
                setErrorMsg('Product already exists in the Cart');
                setSuccessMsg(''); // Clear success message when there's an error
            } else {
                addDoc(collection(db, `Cart-${loggeduser[0].uid}`), {
                    ...
                    .then(() => {
                        setErrorMsg(''); // Clear error message when there's success
    ...
    const addwishlist = async (item) => {
        if (loggeduser) {
            const wishRef = collection(db, `Wishlist-${loggeduser[0].uid}`);
            const wishSnapshot = await getDocs(wishRef);
            const wishProducts = wishSnapshot.docs.map((doc) => doc.data().product);

            // Check if the product already exists in the cart
            const isProductInWishlist = wishProducts.some((item) => item.producttitle === product.producttitle);

            if (isProductInWishlist) {
                setErrorMsg('Product already exists in the wishlist');
                setSuccessMsg(''); // Clear success message when there's an error
            } else {
            ...
            {product ?
                <div className='myprod-container'>
                    <div className='prod-img-cont'>
                        ...
                        <div style={{ justifyContent: 'center', alignItems: "center", display: 'flex', marginTop: 10, marginRight:10}}>
                            <img src={aricon} alt='' style={{ width: 18, height: 20, marginTop: 0, marginRight: 5 }}></img>
                            <a href={product.ARLink} style={{ fontWeight: 'bold', fontSize: 15, color: 'black', marginLeft: 2, textDecoration: 'none' }}>View in AR</a>
                    ...
                    <div className='prod-data'>
                        ...
                        <div className='buy-cart'>
                            ...
                            <button className='btn-wishlist' onClick={addwishlist}> <img src={wishlisticon} alt="no img" /> </button>
